  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (activeTab === "login") {
        ...
        // Auto-login or switch to login tab after register?
        // Let's switch to login tab with success message for now, or just auto-login if backend supports it.
        // Assuming registerUser doesn't return tokens based on previous code.
        setActiveTab("login");
        setError("Account created! Please login."); // Using error state for success msg temporarily or create success state
      }
    } catch (err) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-card border border-border/50 rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-300">
        ...
        <div className="flex p-2 bg-secondary/50 m-6 mb-0 rounded-xl relative">
            <div 
                className={`absolute inset-y-2 w-[calc(50%-8px)] bg-background border border-border/50 shadow-sm rounded-lg transition-transform duration-300 ease-spring ${activeTab === 'login' ? 'left-2 translate-x-0' : 'left-2 translate-x-full'}`} 
            />
            <button
                type="button"
                onClick={() => setActiveTab("login")}
                className={`flex-1 relative z-10 py-2 text-sm font-medium transition-colors duration-200 ${activeTab === 'login' ? 'text-foreground' : 'text-muted-foreground hover:text-foreground'}`}
            >
                Log In
            </button>
            <button
                ...
                className={`flex-1 relative z-10 py-2 text-sm font-medium transition-colors duration-200 ${activeTab === 'register' ? 'text-foreground' : 'text-muted-foreground hover:text-foreground'}`}
            >
                Register
            </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 pt-6 space-y-4">
          ...
          <div className="space-y-2">
           ...
           {error && (
            <div className={`text-xs p-3 rounded-lg ${error.includes("Account created") ? "text-green-600 bg-green-500/10" : "text-destructive bg-destructive/10"}`}>
              {error}
            </div>
          )}
        ...
        <div className="p-6 pt-0 text-center">
            ...
            <div className="mt-4 flex gap-2">
                <button type="button" className="flex-1 h-9 rounded-lg border border-input bg-transparent hover:bg-secondary transition-colors flex items-center justify-center text-muted-foreground hover:text-foreground">
