        ...
        with patch('app.main.performance_monitor') as mock_monitor:
            with patch('app.main.orchestrator_logger') as mock_logger:
                with patch('app.main.graph_app') as mock_graph:
                    ...
                    mock_websocket.send_text.assert_called_with(json.dumps(expected_response))
        ...
        with patch('app.main.performance_monitor') as mock_monitor:
            with patch('app.main.orchestrator_logger') as mock_logger:
                with patch('app.main.graph_app') as mock_graph:
                    ...
                    mock_websocket.send_text.assert_any_call(json.dumps(expected_navigation))
        ...
        with patch('app.main.performance_monitor') as mock_monitor:
            with patch('app.main.orchestrator_logger') as mock_logger:
                with patch('app.main.graph_app') as mock_graph:
                    ...
                    mock_websocket = AsyncMock(spec=WebSocket)
                    mock_websocket.accept.return_value = None
                    mock_websocket.receive_text.side_effect = ["Hello", Exception("Disconnect")]
                    mock_websocket.send_text.return_value = None
        ...
        with patch('app.main.performance_monitor') as mock_monitor:
            with patch('app.main.orchestrator_logger') as mock_logger:
                with patch('app.main.graph_app') as mock_graph:
                    ...
                    mock_websocket = AsyncMock(spec=WebSocket)
                    mock_websocket.accept.return_value = None
                    mock_websocket.receive_text.side_effect = ["Message 1", "Message 2", Exception("Done")]
                    mock_websocket.send_text.return_value = None
                    ...
                    assert mock_logger.info.call_count >= 4  # Connection + 2 messages + 2 responses
