        ...
        for i, card in enumerate(cards):
            try:
                ...
                prize_match = re.search(
                    r"\$[\d,]+(?:\.\d{2})?\s+in\s+prizes", full_text
                )
                data["prize_pool"] = prize_match.group(0) if prize_match else ""

                participants_match = re.search(
                    r"(\d+(?:,\d+)?)\s+participants", full_text
                )
                data["participants"] = (
                    participants_match.group(1) if participants_match else "0"
                )
                ...
                data["organizer"] = (
                    organizer_match.group(1).strip() if organizer_match else ""
                )
                ...
                # Progress
                print(
                    f"{i+1}: {data['title'][:40]}... | {data['mode']} | {data['prize_pool']}"
                )
